...
if(isset($_POST['btn_action']))
{
	if($_POST['btn_action'] == 'Add')
	{
		if( !empty($_POST['category_name']) ){
			...
			$ins_category = $pdo->prepare("INSERT INTO billing_category2 (category_name, category_status) VALUES (?,?)");
	...
	if($_POST['btn_action'] == 'Edit')
	{
		if(!empty($_POST['category_id']) && !empty($_POST['category_name']) ){
			...
			$ins_category = $pdo->prepare("update billing_category2 set category_name = ? where category_id = ?");
